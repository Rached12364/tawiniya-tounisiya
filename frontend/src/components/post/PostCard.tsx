...
export function Avatar({ path, size = 40 }: { path?: string; size?: number }) {
  return (
    <div
      ...
      className="rounded-full bg-navy/10 dark:bg-white/10 overflow-hidden shrink-0 grid place-items-center"
    >
      {path ? (
        <img src={imageUrl(path)} alt="" className="w-full h-full object-cover" />
      ) : (
        <UserIcon size={size * 0.5} className="text-navy/30 dark:text-white/30" />
      )}
    </div>
  );
}
function ReactionPicker({ onPick }: { onPick: (t: ReactionType) => void }) {
  ...
  return (
    <div className="relative" ref={ref}>
      {open && (
        <div className="absolute bottom-full mb-1 start-0 flex gap-1 bg-white dark:bg-[#12283F] rounded-full shadow-lg border border-navy/10 dark:border-white/10 px-2 py-1.5 z-10">
          {REACTIONS.map((r) => (
            <button
              key={r.type}
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 text-sm font-medium text-navy/60 dark:text-white/60 hover:text-teal transition-colors"
      >
...
function CommentReactionPicker({ onPick }: { onPick: (t: ReactionType) => void }) {
  ...
  return (
    <div className="relative inline-block" ref={ref}>
      {open && (
        <div className="absolute bottom-full mb-1 start-0 flex gap-1 bg-white dark:bg-[#12283F] rounded-full shadow-lg border border-navy/10 dark:border-white/10 px-2 py-1 z-10">
          {REACTIONS.map((r) => (
            <button
              key={r.type}
              onClick={() => { onPick(r.type); setOpen(false); }}
      ...
      <button onClick={() => setOpen((v) => !v)} className="text-xs font-semibold text-navy/50 dark:text-white/50 hover:text-teal transition-colors">
        J'aime
      </button>
    </div>
  );
  ...
  return (
    <div className="flex gap-2.5 items-start group">
      ...
      <div className="flex-1 min-w-0">
        <div className="bg-navy/[0.03] dark:bg-white/5 rounded-xl px-3 py-2">
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs font-semibold text-navy dark:text-white">{comment.author.prenom} {comment.author.nom}</p>
          ...
          <p className="text-sm text-navy/80 dark:text-white/80 mt-0.5 whitespace-pre-line">{comment.content}</p>
        </div>
        <div className="flex items-center gap-3 mt-1 px-1">
          ...
          {depth === 0 && (
            <button onClick={() => setReplying((v) => !v)} className="text-xs font-semibold text-navy/50 dark:text-white/50 hover:text-teal transition-colors">
              Répondre
            </button>
          )}
          {comment.totalReactions > 0 && (
            <span className="flex items-center gap-1 text-xs text-navy/40 dark:text-white/40">
        ...
        {replying && (
          <div className="flex gap-2 items-center mt-2">
            ...
            <input
              ...
              className="flex-1 rounded-full border border-navy/15 dark:border-white/15 bg-transparent px-3 py-1 text-xs text-navy dark:text-white placeholder:text-navy/40 dark:placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-teal/40 focus:border-teal"
            />
        ...
        {comment.replies && comment.replies.length > 0 && (
          <div className="mt-2 pl-2 border-l-2 border-navy/10 dark:border-white/10 flex flex-col gap-2">
...
function PostContent({ content }: { content: string }) {
  ...
  return (
    <p className="mt-3 text-sm text-navy dark:text-white whitespace-pre-line">
      {displayed}
      {isLong && (
        <>
          {' '}
...
function PostComments({ postId }: { postId: number }) {
  ...
  return (
    <div className="mt-3 pt-3 border-t border-navy/10 dark:border-white/10 flex flex-col gap-3">
      {loading ? (
        <Loader2 size={16} className="animate-spin text-navy/30 dark:text-white/30" />
      ) : (
        comments.map((c) => (
          <CommentItem
            key={c.id}
      ...
      <div className="flex gap-2.5 items-center">
        ...
        <input
          ...
          className="flex-1 rounded-full border border-navy/15 dark:border-white/15 bg-transparent px-3.5 py-1.5 text-sm text-navy dark:text-white placeholder:text-navy/40 dark:placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-teal/40 focus:border-teal"
        />
...
function PostMenu({ post, onEdit, onDelete, onTogglePin, onToggleSave }: {
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen((v) => !v)} className="text-navy/30 dark:text-white/30 hover:text-navy/60 dark:hover:text-white/60 transition-colors p-1">
        <MoreHorizontal size={18} />
      </button>
      {open && (
        <div className="absolute end-0 top-full mt-1 w-52 rounded-lg bg-white dark:bg-[#12283F] shadow-xl border border-navy/10 dark:border-white/10 overflow-hidden z-20 py-1">
          <button onClick={() => { onToggleSave(); setOpen(false); }} className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm text-navy dark:text-white hover:bg-navy/5 dark:hover:bg-white/5 transition-colors">
            <Bookmark size={15} className={post.savedByMe ? 'fill-teal text-teal' : ''} />
            {post.savedByMe ? 'Retirer des enregistrés' : 'Enregistrer'}
          </button>
          <button onClick={copyLink} className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm text-navy dark:text-white hover:bg-navy/5 dark:hover:bg-white/5 transition-colors">
            {copied ? <Check size={15} className="text-teal" /> : <LinkIcon size={15} />}
            {copied ? 'Lien copié !' : 'Copier le lien'}
          </button>
          {post.canEdit && (
            <>
              <button onClick={() => { onTogglePin(); setOpen(false); }} className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm text-navy dark:text-white hover:bg-navy/5 dark:hover:bg-white/5 transition-colors">
                <Pin size={15} className={post.pinned ? 'fill-gold text-gold' : ''} />
                {post.pinned ? 'Désépingler du profil' : 'Épingler en haut du profil'}
              </button>
              <button onClick={() => { onEdit(); setOpen(false); }} className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm text-navy dark:text-white hover:bg-navy/5 dark:hover:bg-white/5 transition-colors">
...
export default function PostCard({ post, onChanged, onRemoved }: {
  ...
  return (
    <div className="bg-white dark:bg-[#12283F] rounded-xl shadow-sm dark:shadow-black/30 p-4 mb-4">
      {post.pinned && (
      ...
      <div className="flex items-start justify-between">
        <Link to={`/profil/${post.author.id}`} className="flex items-center gap-2.5 group">
          ...
          <div>
            <p className="text-sm font-semibold text-navy dark:text-white group-hover:text-teal transition-colors">{fullName}</p>
            <p className="text-xs text-navy/40 dark:text-white/40">{ROLE_LABELS[post.author.role]} · {timeAgo(post.createdAt)}</p>
          </div>
        </Link>
        <PostMenu
          post={post}
      ...
      {editing ? (
        <div className="mt-3">
          <textarea
            ...
            rows={3}
            className="w-full rounded-lg border border-navy/15 dark:border-white/15 bg-transparent px-3 py-2 text-sm text-navy dark:text-white placeholder:text-navy/40 dark:placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-teal/40 focus:border-teal resize-none"
          />
      ...
      {post.totalReactions > 0 && (
        <div className="flex items-center gap-1.5 mt-3 text-xs text-navy/50 dark:text-white/50">
      ...
      <div className="flex items-center gap-5 mt-2 pt-2 border-t border-navy/10 dark:border-white/10">
        {post.myReaction ? (
