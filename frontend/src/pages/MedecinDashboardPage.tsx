...
type Tab = 'dashboard' | 'consultations' | 'profil' | 'actualites' | 'evenements';
const TABS: { key: Tab; label: string; icon: typeof LayoutDashboard }[] = [
  { key: 'dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { key: 'consultations', label: 'Consultations', icon: MessageSquareWarning },
];
...
function ProfilTab({ user, onSaved }: { user: User; onSaved: (u: User) => void }) {
  ...
  const fields: { key: keyof ProfilFormState; label: string; textarea?: boolean }[] = [
    ...
    { key: 'adresse', label: 'Adresse du cabinet médical' },
...
function ConsultationsChart({ conversations }: { conversations: ExpertConversationSummary[] }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-bold text-teal uppercase tracking-wide">Consultations reçues</h3>
      ...
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
          ...
          />
          <Bar dataKey="count" name="Consultations" fill="#14b8a6" radius={[4, 4, 0, 0]} maxBarSize={36} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
...
export default function MedecinDashboardPage() {
  ...
  useEffect(() => {
    if (tab === 'consultations') loadConversations();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tab]);
  ...
  const fullName = user ? `${user.prenom} ${user.nom}`.trim() : 'Médecin';
        ...
        <div className="flex items-center gap-3 px-5 h-20 shrink-0 bg-navy-dark border-b border-white/10">
          ...
          {!collapsed && (
            <div className="min-w-0">
              ...
              <p className="text-[10px] font-semibold text-gold uppercase tracking-widest truncate">Médecin</p>
            </div>
          )}
        </div>
      ...
      <main className={`flex-1 min-h-screen transition-all duration-200 ${collapsed ? 'ml-20' : 'ml-64'}`}>
        ...
        <div className="px-8 -mt-10 mb-6 flex items-end gap-4">
          ...
          <div className="pb-1">
            ...
            <p className="text-sm text-navy/50">{user?.gouvernorat ?? ''} — Médecin CTTEERA</p>
          </div>
        </div>
        {/* Contenu */}
        <div className="px-8 pb-16">
          ...
          {tab === 'dashboard' ? (
            <div className="flex flex-col gap-6">
              <ConsultationsChart conversations={conversations} />
              ...
              <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-5 py-3.5 border-b border-navy/10">
                  <h3 className="text-sm font-bold text-teal uppercase tracking-wide">Dernières consultations</h3>
                  {conversations.length > 0 && (
                    <button onClick={() => setTab('consultations')} className="text-xs font-semibold text-navy/50 hover:text-teal transition-colors">
                      Voir tout
                    </button>
                  )}
                </div>
                {conversationsLoading ? (
                  ...
                  <div className="flex flex-col">
                    {conversations.slice(0, 5).map((conv) => (
                      <button
                        key={conv.id}
                        onClick={() => { setSelectedConversationId(conv.id); setTab('consultations'); }}
