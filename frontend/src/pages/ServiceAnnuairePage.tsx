...
interface ServiceAnnuairePageProps {
  targetRole: Role;
  title: string;
  subtitle: string;
  providerLabel: string;
}
function ProviderRow({ card, providerLabel }: { card: UserCard; providerLabel: string }) {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5">
          <p className="text-xs text-navy/50 truncate">{card.subtitle || providerLabel}</p>
...
export default function ServiceAnnuairePage({ targetRole, title, subtitle, providerLabel }: ServiceAnnuairePageProps) {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    browseByRole(targetRole, 0, 100)
      .then((res) => setCards(res.content))
      .catch(() => setError(`Impossible de charger les ${providerLabel.toLowerCase()}s.`))
      .finally(() => setLoading(false));
  }, [targetRole, providerLabel]);
  ...
  const selectedConversation = conversations.find((c) => c.otherUser.id === selectedProviderId);
  const selectedProviderCard = cards.find((c) => c.id === selectedProviderId);
  const selectedName = selectedConversation
    ? `${selectedConversation.otherUser.prenom} ${selectedConversation.otherUser.nom}`
    : selectedProviderCard
    ? `${selectedProviderCard.prenom} ${selectedProviderCard.nom}`
    : '';
  return (
          ...
          {conversationsLoading ? (
            ...
            <div className="flex flex-col gap-2">
              {conversations.map((conv) => (
                <div
                  key={conv.id}
                  onClick={() => setSelectedProviderId(conv.otherUser.id)}
                  className={`bg-white rounded-xl border px-3 py-2.5 flex items-center gap-2.5 cursor-pointer transition-all ${
                    selectedProviderId === conv.otherUser.id ? 'border-teal shadow-sm' : 'border-navy/10 hover:border-teal/30 hover:shadow-sm'
                  }`}
                >
        ...
        <div className="flex-1 min-w-0">
          {selectedProviderId ? (
            <div>
              ...
              <ExpertChatWidget key={selectedProviderId} expertId={selectedProviderId} />
            </div>
          ) : (
            <div>
              <h1 className="text-2xl font-black text-navy mb-1">{title}</h1>
              <p className="text-sm text-navy/50 mb-6">{subtitle}</p>
              <div className="flex flex-wrap items-center gap-3 mb-5">
                <div className="relative flex-1 min-w-[220px]">
                  ...
                  <input
                    ...
                    placeholder={`Rechercher un ${providerLabel.toLowerCase()}, une adresse...`}
              ...
              {loading ? (
                ...
                <p className="text-navy/50 text-sm">
                  {search || filterAdresse ? 'Aucun résultat pour ces critères.' : `Aucun ${providerLabel.toLowerCase()} pour le moment.`}
                </p>
              ) : (
                <div className="flex flex-col gap-2.5">
                  {filtered.map((c) => (
                    <ProviderRow key={c.id} card={c} providerLabel={providerLabel} />
                  ))}
                </div>
              )}
            </div>
